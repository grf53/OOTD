...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct AbsoluteDurationRange {
    pub(crate) min_seconds: i64,
    pub(crate) max_seconds: i64,
}
...
fn from_i128_range(min: i128, max: i128) -> Option<AbsoluteDurationRange> {
    if min > max {
        return None;
    }

    Some(AbsoluteDurationRange {
        min_seconds: i64::try_from(min).ok()?,
        max_seconds: i64::try_from(max).ok()?,
    })
}
