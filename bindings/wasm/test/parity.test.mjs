...
import { between, fromDuration } from '../pkg/ootd_wasm.js'

const dts = readFileSync(resolve(process.cwd(), 'pkg/ootd_wasm.d.ts'), 'utf-8')
...
for (const c of fixture.duration_cases) {
  if (c.expected_error) {
    let failed = false
    try {
      fromDuration(BigInt(c.seconds), c.is_future, c.locale, c.use_native_ko_number ?? false)
    } catch (e) {
  ...
  const out = fromDuration(BigInt(c.seconds), c.is_future, c.locale, c.use_native_ko_number ?? false)
  if (out !== c.expected) {
